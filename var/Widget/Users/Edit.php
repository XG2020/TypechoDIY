...
class Widget_Users_Edit extends Widget_Abstract_Users implements Widget_Interface_Do
{
    ...
    public function form($action = NULL)
    {
        ...
        /** 用户签名 */
        $sign = new Typecho_Widget_Helper_Form_Element_Text('sign', NULL, NULL, _t('用户签名'), _t('修改用户签名, 用于前台显示.')
            . '<br />' . _t('如果你将此项留空, 将默认签名为空.'));
        $form->addInput($sign);

        /** 用户背景 */
        $bgimg = new Typecho_Widget_Helper_Form_Element_Text('bgimg', NULL, NULL, _t('用户背景'), _t('修改用户背景图片, 用于前台显示.')
            . '<br />' . _t('如果你将此项留空, 将使用默认图片.'));
        $form->addInput($bgimg);
        ...
        if ('insert' == $action || 'update' == $action) {
            ...
            $sign->addRule('maxLength',_t('个性签名最多包含200个字符'),200);
            $bgimg->addRule('url', _t('图片地址格式错误'));
    ...
    public function updateUser()
    {
        ...
        /** 取出数据 */
        $user = $this->request->from('mail', 'screenName', 'sign', 'bgimg', 'password', 'url', 'group');
        $user['screenName'] = empty($user['screenName']) ? $user['name'] : $user['screenName'];
